...
class WriteOffService:
    ...
    def apply(self, *, member_id: UUID, business_ref: str, event_type: str, user: CurrentUser, idempotency_key: str, trace_id: str, today: date) -> WriteOffEvent:
        ...
        if member.status in {"disabled", "paused", "expired"}:
            raise HTTPException(status_code=409, detail="Member status does not allow write-off")
        if event_type == "reserve_hold":
            return self._reserve(member_id, business_ref, user, idempotency_key, trace_id, today)
        if event_type not in TERMINAL_EVENT_TYPES:
            raise HTTPException(status_code=422, detail="Unsupported write-off event")
        return self._terminal(member_id, business_ref, event_type, user, idempotency_key, trace_id)

    def _reserve(self, member_id, business_ref, user, key, trace_id, today):
        cards = self.card_repo.list_fefo_candidates(member_id, today, for_update=True, include_pending=True)
        if not cards:
    ...
    def _terminal(self, member_id, business_ref, event_type, user, key, trace_id):
    ...
    def _audit(self, event, card, user, before, trace_id, key):
        record_audit(
